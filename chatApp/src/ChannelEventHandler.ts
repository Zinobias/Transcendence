...
export class ChannelEventHandler {
  ...
  channelCreate(data: ChannelCreate) {
    let channel: Channel;
    let user: User = ChannelEventHandler.getUser(data.creator_id, "channel_create")
    if (user == null)
      return;
    const usersArr: User[] = [User.getUser(data.creator_id)]

    if (data.creator2_id != -1) {
      let user2: User = ChannelEventHandler.getUser(data.creator2_id, "channel_create")
      if (user2 == null)
        return;
      usersArr.push(user2)
    }
    ...
    if (channelId == -1) {
      Logger.warn("Received invalid channel [" + channelId + "] from channel_create")
      return;
    }
  ...
  handleJoin(data: ChannelJoin) {
    let channel: Channel = ChannelEventHandler.getChannel(data.channel_id, "channel_join");
    if (channel == null)
      return;
    let user: User = ChannelEventHandler.getUser(data.user_id, "channel_join")
    if (user == null)
      return;
  ...
  handleLeave(data: ChannelLeave) {
    let channel: Channel = ChannelEventHandler.getChannel(data.channel_id, "channel_leave");
    if (channel == null)
      return;
    if (ChannelEventHandler.userInChannel(channel, data.user_id, "channel_leave", true))
      return;
  ...
  handlePromote(data: ChannelPromote) {
    let channel: Channel = ChannelEventHandler.getChannel(data.channel_id, "channel_promote");
    if (channel == null)
      return;
    if (ChannelEventHandler.userInChannel(channel, data.user_id, "channel_promote"))
      return;

    let setting: Setting = new Setting(SettingType.ADMIN, data.channel_id, data.user_id, data.actor_id, new Date().getTime(), -1);
  ...
  handleDemote(data: ChannelDemote) {
    let channel: Channel = ChannelEventHandler.getChannel(data.channel_id, "channel_demote");
    if (channel == null)
      return;
    if (ChannelEventHandler.userInChannel(channel, data.user_id, "channel_demote", true))
      return;
  ...
  handleKick(data: ChannelKick) { //TODO send a message to the frontend to notify kicked user somewhere (if we want to do that?)
    let channel: Channel = ChannelEventHandler.getChannel(data.channel_id, "channel_kick");
    if (channel == null)
      return;
    if (ChannelEventHandler.userInChannel(channel, data.user_id, "channel_kick", true))
      return;
  ...
  handleBan(data: ChannelBan) { //TODO send a message to the frontend to notify banned user somewhere (if we want to do that?)
    let channel: Channel = ChannelEventHandler.getChannel(data.channel_id, "channel_ban");
    if (channel == null)
      return;
    if (ChannelEventHandler.userInChannel(channel, data.user_id, "channel_ban", true))
      return;
    ...
    let setting: Setting = new Setting(SettingType.BANNED, data.channel_id, data.user_id, data.actor_id, new Date().getTime(), data.until);
    channel.addSetting(setting)
    Queries.getInstance().addSetting(setting)
  }
  ...
  handleDisband(data: ChannelDisband) { //TODO send a message to the frontend to notify all other users (if we want to do that?)
    let channel: Channel = ChannelEventHandler.getChannel(data.channel_id, "channel_disband");
    if (channel == null)
      return;
  ...
  private static getChannel(channelId: number, source: string): Channel {
    let channel: Channel = Channel.getChannel(channelId);
    if (channel == null) {
      Logger.warn("Received invalid channel id [" + channelId + "] from " + source)
      return undefined;
    }
    return channel;
  }
   ...
   * @private
   */
  private static userInChannel(channel: Channel, userId: number, source: string, invert?: boolean): boolean {
    if (invert) {
    ...
    if (channel.hasUser(userId)) {
      Logger.warn("User [" + userId + "] should not be in channel for " + source)
      return true;
    }
    return false;
  }

  private static notAdmin(channel: Channel, actorId: number, source: string): boolean {
    if (!channel.isAdmin(actorId)) {
      Logger.warn("Actor [" + actorId + "] is not an admin and can't issue the request " + source)
      return true;
    }
    return false;
  }

  private static notOwner(channel: Channel, actorId: number, source: string): boolean {
    if (!channel.isAdmin(actorId)) {
      Logger.warn("Actor [" + actorId + "] is not an owner and can't issue the request " + source)
      return true;
    }
    return false;
  }
