...
export class ChannelEventPatterns {
  ...
  async channelCreate(data: ChannelCreate) {
    const user: User = await this.util.getUser(data.user_id, 'channel_create');
    if (user == null) {
      this.emitFailedObject(
        data.user_id,
        'channel_create',
        'Unable to find dm user',
      );
      return;
    }
    ...
    if (data.creator2_id != undefined) {
      user2 = await this.util.getUser(data.creator2_id, 'channel_create');
      if (user2 == null) {
        this.emitFailedObject(
          data.user_id,
          'channel_create',
          'Unable to find second dm user',
        );
        return;
      }
      usersArr.push(user2);
  ...
  async handlePasswordChange(data: ChannelUpdatePassword) {
    ...
    if (channel == null) {
      this.emitFailedObject(
        data.user_id,
        'channel_update_password',
        `Unable to find the channel you're trying to join`,
      );
      return;
    }
    ...
    if (data.password === undefined || data.password.length != 64) {
      //This only works for a sha256 hash
      this.emitFailedObject(
        data.user_id,
        'channel_update_password',
  ...
  async handleLeave(data: ChannelLeave) {
    ...
    await Queries.getInstance().removeChannelMember(
      data.channel_id,
      data.user_id,
    );
  ...
  async handleDemote(data: ChannelDemote) {
    ...
    if (channel == null) return;
    if (
      this.util.userInChannel(channel, data.affected_id, 'channel_demote', true)
    )
      return;
    if (this.util.notAdmin(channel, data.affected_id, 'channel_kick')) return;
  ...
  async handleKick(data: ChannelKick) {
    ...
    if (channel == null) return;
    if (
      this.util.userInChannel(channel, data.affected_id, 'channel_kick', true)
    )
      return;
    if (this.util.notOwner(channel, data.affected_id, 'channel_kick')) return;
  ...
  async handleBan(data: ChannelBan) {
    ...
    if (this.util.userInChannel(channel, data.affected_id, 'channel_ban', true))
      return;
    if (this.util.notAdmin(channel, data.affected_id, 'channel_ban')) return;
  ...
  async handleRetrieve(data: ChannelsRetrieve) {
    ...
    this.util.notify(
      [data.user_id],
      'channels_retrieve',
      (await Queries.getInstance().getAllPublicChannels()).map((channel) => {
        channel.getIChannel();
      }),
    );
  }
