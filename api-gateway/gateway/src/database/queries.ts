...
export class Queries {
  ...
  public async storeAuth(id: number, auth: string): Promise<boolean> {
    ...
    if (!(await this.userExists(id)) === true)
      //Check if the user is in the database, they can't get a session if they're not
      return false;
  ...
  public async createUser(
    ...
    try {
      ...
      Logger.warn(
        `Unable to run save user query for [${userId}] see error: ${e}`,
      );
      return `Unknown error while saving the user in the database`;
    }
    return true;
