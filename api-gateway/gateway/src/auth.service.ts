...
export class Auth {
  ...
  public validate(
    ...
    if (userId === undefined || accessToken === undefined) {
      this.logger.warn(
        `Received undefined userId [${userId}] or accessToken [${accessToken}] when validating auth`,
      );
      return false;
    }

    let specialGetFindsAuth = false;
    this.map.forEach((value, key) => {
      if (key != userId) {
        return;
      }
      const result = value.find((val) => {
  ...
  public async updateAuth(userId: number) {
    ...
    if (accessToken != undefined) {
      this.logger.debug(
        `Storing session token for user: [${userId}] token: [${accessToken}]`,
      );
      this.map.set(userId, accessToken);
    } else
      Logger.warn(
        `Received undefined accessToken when loading session for user id [${userId}]`,
      );
  }
  ...
  private async auth(
    ...
    this.logger.log(
      `Received ${signup ? 'signup' : 'login'} event with token [${token}]`,
    );
  ...
  public async login(
    ...
    if ((await this.TFA.hasTwoFA(userId))) {
      if (TFAToken && (await this.TFA.verify(userId, TFAToken))) {
        return {
          user_id: userId,
          auth_cookie: uuid,
        };
  ...
  private async storeSession(userId: number, uuid: string): Promise<boolean> {
    ...
    else this.logger.warn(`Unable to store auth for user id [${userId}]`);
    return isSuccessful;
  }
  ...
  public async createAccount(
    ...
    const createUserResult: string | boolean = await this.queries.createUser(
      userId,
      payload.userName,
    );
    if (typeof createUserResult == 'string') return createUserResult as string;
    this.sockets.storeSocket(userId, client);
    const uuid = randomUUID();
    if (await this.storeSession(userId, uuid))
      return { user_id: userId, auth_cookie: uuid };
